...
## The Taser Boss script handles the behavior and attacks of the Taser Boss entity.
##
## The boss moves around the room, attacking the player with various projectile patterns.
## The boss has a health bar and can spawn forcefields and buttons to interact with.
## The boss can perform a variety of attacks, each with different patterns and effects.
## The boss can also trigger quick succession attacks for added challenge.

## The animation player for the boss entity.
@onready var _animated_sprite = $AnimatedSprite2D

## The scene for the electro laser entity.
@export var electro_laser_scene: PackedScene

## Scene for the taser projectile
@export var taser_projectile_scene: PackedScene

## Scene for the button entity
@export var button_scene: PackedScene

## Scene for the electric forcefield entity
@export var electric_forcefield_scene: PackedScene

## The health bar entity for the boss.
...
func _create_projectile(projectile: Node) -> void:
	...
	if not taser_proj:
		push_error("Projectiles node not found! Adding node to scene tree, but add a Projectiles node to the scene tree for proper organization.")
		taser_proj = Node.new()
...
func attack_spawn_buttons_with_forcefield() -> void:
	...
	for pos in button_positions:
		...
		print("Spawned button at pos: ", pos)
...
func take_damage(amount: float) -> void:
	...
	if (float(_current_health) / float(max_health)) <= 0.5:
		for attack in attacks:
			if attack.unlocked == false:
				attack.unlocked = true

	health_bar.health = _current_health
